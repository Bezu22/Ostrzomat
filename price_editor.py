...
class PriceEditor(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        ctk.CTkButton(self.top_bar, text="+ DODAJ", fg_color="#28a745", width=100, command=lambda: self.open_edit_form(None)).pack(side="right", padx=10)
    ...
    def refresh_list(self, _=None):
        """Czyści listę, pobiera dane i resetuje scrollbar."""
        self.selected_row_data = None
        self.btn_edit.configure(state="disabled")
        self.btn_delete.configure(state="disabled")
        
        # 1. Usuwanie fizycznych wierszy
        for child in list(self.scroll_frame.winfo_children()): child.destroy()
        
        # 2. NAPRAWA BŁĘDU: Przewinięcie widoku na samą górę
        # ScrollableFrame zawiera wewnętrzny Canvas, który musimy zresetować
    ...
    def open_edit_form(self, existing_data=None):
        ...
        for i, txt in enumerate(labels):
            ...
            if data: e.insert(0, str(data[db_slice][i]))
            e.pack(pady=5)
            entries.append(e)
        ...
        def save_action():
            try:
                ...
                if cat == "Narzędzia":
                    if data: database.update_tool_row(data[0], vals)
                    else: database.add_tool_row(vals)
                elif cat == "Powłoki":
                    if data: database.update_coating_row(data[0], vals)
                    else: database.add_coating_row(vals)
                else:
                    if data: database.update_service_row(data[0], vals)
                    else: database.add_service_row(vals)
                form.destroy()
                self.refresh_list()
            except Exception as e: messagebox.showerror("Błąd", str(e))

        ctk.CTkButton(form, text="ZAPISZ", fg_color="#28a745", command=save_action).pack(pady=20)